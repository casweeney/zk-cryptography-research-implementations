use crate::basic_sumcheck::prover::{field_element_to_bytes, SumcheckProof};
...
impl<F: PrimeField> Verifier<F> {
    ...
    pub fn verify(&mut self, proof: SumcheckProof<F>) -> bool {
        ...
        for i in 0..proof.round_univariate_polynomials.len() {
            ...
            if proof.round_univariate_polynomials[i].evaluate(&eval_at_zero)
                + proof.round_univariate_polynomials[i].evaluate(&eval_at_one)
                != current_claim_sum
            {
                return false;
            }
