...
mod tests {
    ...
    fn test_multilinear_kzg() {
        ...
        let commitment = MultilinearKZG::commit_to_polynomial(&polynomial, &setup);
        let opening_values = vec![Fr::from(6), Fr::from(4), Fr::from(0)];

        let proof = MultilinearKZG::open_and_prove(&polynomial, &setup, &opening_values.to_vec());

        let verification = MultilinearKZG::verify(&setup, &commitment, &opening_values, &proof);
